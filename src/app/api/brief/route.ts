...
export async function POST(request: Request) {
  ...
  if (!isMarket(payload.market)) return Response.json({ error: messages.brief.invalidPayload }, { status: 400 });
  ...
  const report = buildBriefReport(parsed.answers, recommendedRoute, messages, locale, payload.market);
